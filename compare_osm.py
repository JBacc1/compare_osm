...
def add2add_file(file,object_type,id,tag_list):
	...
	for tag in tag_list:
		if tag[0]=="name": line+=tag[0].strip()+ "=" + tag[1].replace(" ","_")+" "
...
print("Chargement : "+file_pre)
osm_pre.load_xml_file(file_pre)
print("Chargement : "+file_post)
...
#NODES
	for node_post in osm_post.find_nodes(lambda x: True):
		count+=1
		if VERBOSE and count%2500==0: print(str(count)+" éléments traités")
